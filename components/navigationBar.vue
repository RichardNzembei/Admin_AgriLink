...
// Map routes to icons (using @nuxt/icon syntax from earlier)
const navItems = [
  { to: '/dashboard', label: 'Overview', icon: 'i-lucide-home' },
  { to: '/reports', label: 'Reports', icon: 'i-lucide-bar-chart' },
  { to: '/management', label: 'Management', icon: 'i-lucide-users' },
  { to: '/shares', label: 'Shares', icon: 'i-lucide-pie-chart' },
  ...
  <header class="bg-white p-4 rounded-lg shadow mb-6">
    ...
    <nav class="hidden md:flex justify-center gap-6 mt-4">
      <RouterLink
        ...
        :key="item.to"
        :to="item.to"
        class="flex items-center gap-2 px-3 py-2 rounded-md font-medium transition-colors duration-200"
        :class="{
    ...
    <nav
      v-if="isMenuOpen"
      class="md:hidden mt-4 flex flex-col gap-2 bg-white p-4 rounded-lg shadow"
    >
      <RouterLink
        v-for="item in navItems"
        :key="item.to"
